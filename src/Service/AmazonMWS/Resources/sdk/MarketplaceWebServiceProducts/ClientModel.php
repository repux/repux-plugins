...
abstract class ClientModel
{
        ...
        foreach ($this->_fields as $fieldName => $field) {
            ...
            if (is_array($fieldType)) {
                if ($fieldType[0] == "object") {
                    ...
                    if ($this->_isComplexType($fieldType[0])) {
                        if (isset($field['ListMemberName'])) {
                            ...
                            $elements = $xpath->query("./*[local-name()='$fieldName']/*[local-name()='$memberName']",
                                $dom);
                        } else {
                            $elements = $xpath->query("./*[local-name()='$fieldName']", $dom);
                        }
                        if ($elements->length >= 1) {
                            $fieldTypeClassName = PRODUCTS_MODEL_NAMESPACE.$fieldType[0];
                            foreach ($elements as $element) {
                                $this->_fields[$fieldName]['FieldValue'][] = new $fieldTypeClassName($element);
                            }
                        }
                    } else {
                        if (isset($field['ListMemberName'])) {
                ...
                if ($this->_isComplexType($fieldType)) {
                    $elements = $xpath->query("./*[local-name()='$fieldName']", $dom);
                    if ($elements->length == 1) {
                        $fieldTypeClassName = PRODUCTS_MODEL_NAMESPACE.$fieldType;
                        $this->_fields[$fieldName]['FieldValue'] = new $fieldTypeClassName($elements->item(0));
                    }
                } else {
                    if ($fieldType[0] == "@") {
        ...
        foreach ($this->_fields as $fieldName => $field) {
            ...
            if (is_array($fieldType)) {
                ...
                if ($this->_isComplexType($fieldType)) {
                    if (array_key_exists($fieldName, $array)) {
                        $fieldTypeClassName = PRODUCTS_MODEL_NAMESPACE.$fieldType;
                        $this->_fields[$fieldName]['FieldValue'] = new $fieldTypeClassName($array[$fieldName]);
                    }
                } else {
                    if (array_key_exists($fieldName, $array)) {
    ...
    {
        return class_exists(PRODUCTS_MODEL_NAMESPACE.$fieldType);
    }
