...
abstract class ClientModel
{
            ...
            if (is_array($fieldType)) {
                if ($this->isComplexType($fieldType[0])) {
                    ...
                    if ($elements->length >= 1) {
                        $fieldTypeClassName = MODEL_NAMESPACE.$fieldType[0];
                        foreach ($elements as $element) {
                            $this->fields[$fieldName]['FieldValue'][] = new $fieldTypeClassName($element);
                        }
                    }
                } else {
                    $elements = $xpath->query("./a:$fieldName", $dom);
                ...
                if ($this->isComplexType($fieldType)) {
                    $elements = $xpath->query("./a:$fieldName", $dom);
                    if ($elements->length == 1) {
                        $fieldTypeClassName = MODEL_NAMESPACE.$fieldType;
                        $this->fields[$fieldName]['FieldValue'] = new $fieldTypeClassName($elements->item(0));
                    }
                } else {
        ...
        foreach ($this->fields as $fieldName => $field) {
            ...
            if (is_array($fieldType)) {
                ...
                if ($this->isComplexType($fieldType)) {
                    if (array_key_exists($fieldName, $array)) {
                        $fieldTypeClassName = MODEL_NAMESPACE.$fieldType;
                        $this->fields[$fieldName]['FieldValue'] = new $fieldTypeClassName($array[$fieldName]);
                    }
                } else {
                    if (array_key_exists($fieldName, $array)) {
    ...
    {
        return class_exists(MODEL_NAMESPACE.$fieldType);
    }
